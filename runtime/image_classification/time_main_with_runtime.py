...
def main():
    ...
    # Data loading code
    print("Start of data loading: ",datetime.now(tz_NY).strftime("%H:%M:%S.%f")[:-3])
    data_loading_start = time.time()
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                     std=[0.229, 0.224, 0.225])
    print("elapsed time for Normalize: ", time.time() - data_loading_start)
    if args.arch == 'inception_v3':
    ...
    print("End of data pre-processing :", time.time() - data_loading_start)
...
def train(train_loader, r, optimizer, epoch):
    ...
    data_loading_start=time.time()
    print("Data loading start time for stage: ", r.stage,datetime.now(tz_NY).strftime("%H:%M:%S.%f")[:-3])
    r.set_loader(train_loader)
    data_loading_end=time.time()
    print("Data loading end time for stage: ", r.stage, datetime.now(tz_NY).strftime("%H:%M:%S.%f")[:-3])
    print("Elapsed data loading time: ", data_loading_end-data_loading_start)	
